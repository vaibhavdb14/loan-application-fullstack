...
import { useToast } from "../context/ToastContext"; // Imported the toast context

const Sidebar = ({ isOpen = false, onClose = () => {} }) => {
  const navigate = useNavigate();
  const { showToast } = useToast(); // Initialized the toast function
  ...
  const bottomItems = [
    { name: "Manager Profile", icon: <User size={20} /> },
    { name: "Settings", icon: <Settings size={20} /> },
  ];

  // Handler for under-development features
  const handleUnderDevelopment = (featureName) => {
    onClose();
    showToast(`${featureName} is currently under development.`, "info");
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-4 border-t border-gray-700 space-y-1">
          {/* Changed NavLinks to buttons for Settings and Profile */}
          {bottomItems.map((item) => (
            <button
              key={item.name}
              onClick={() => handleUnderDevelopment(item.name)}
              className="w-full flex items-center gap-3 px-4 py-3 sm:py-2 text-left text-gray-300 hover:text-white transition-colors"
            >
              {item.icon}
              <span className="font-medium text-sm">{item.name}</span>
            </button>
